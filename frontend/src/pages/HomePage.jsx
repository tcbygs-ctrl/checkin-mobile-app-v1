...
import { LogIn, LogOut, MapPin, Clock, AlertTriangle, CheckCircle2 } from 'lucide-react';
...
export default function HomePage() {
  ...
  useEffect(() => {
    Promise.all([api.get('/employees/me'), api.get('/checkin/today')])
      .then(([empRes, todayRes]) => { setEmployee(empRes.data); setToday(todayRes.data); })
  ...
  return (
    <div className="page">
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: 'var(--muted)', fontSize: 13 }}>
          รหัส: {employee?.employee_id} · {employee?.departments?.name}
        </p>
      </div>
      ...
      {employee && !employee.face_registered && (
        ...
        >
          ...
          ยังไม่ได้ลงทะเบียนใบหน้า — แตะที่นี่เพื่อลงทะเบียน
        </div>
      )}
      ...
      <button className="btn btn-primary" style={{ fontSize: 16, gap: 10 }} onClick={() => navigate('/checkin')}>
        {!today?.checkin_at
          ...
          : !today?.checkout_at
            ? <><LogOut size={20} /> ออกงาน</>
            : <><CheckCircle2 size={20} /> ดูประวัติวันนี้</>
        }
      </button>
